"""utils, a better directory iterator and faster os.walk(), now in the Python 3.5 stdlib
...
if _scandir is None and ctypes is None:
    import warnings
    warnings.warn("utils can't find the compiled _scandir C module "
...
if IS_PY3 and sys.platform == 'win32':
    def scandir_generic(path=unicode('.')):
        if isinstance(path, bytes):
            raise TypeError("os.utils() doesn't support bytes path on Windows, use Unicode instead")
...
def _walk(top, topdown=True, onerror=None, followlinks=False):
    ...
    if topdown:
        ...
        for name in dirs:
            ...
            # entry.is_symlink() result during the loop on os.utils() because
